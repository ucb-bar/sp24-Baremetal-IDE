...
void pwm_set_frequency(PWM_Type *PWMx, uint32_t idx, uint32_t freq) {
  ...
  uint16_t pwmscale = (int) (log2_bitwise( (int) (((double) sys_clk_freq / ((double) freq))))) + 1; //65535 = 2^16-1
  ...
  if (READ_BITS(PWMx->PWM_CFG, PWM_PWMZEROCMP_MSK) != 0) {
    uint16_t cmp0 = (double) sys_clk_freq / (1<<pwmscale);
    //printf("CMP0 %d", cmp0);
...
void pwm_set_duty_cycle(PWM_Type *PWMx, uint32_t idx, uint32_t duty, int phase_corr) {
  ...
  uint16_t zecmp = READ_BITS(PWMx->PWM_CFG, PWM_PWMZEROCMP_MSK);
  if (READ_BITS(PWMx->PWM_CFG, PWM_PWMZEROCMP_MSK) == 0){
     cmpvalue = ((double) duty/100) * (double) sys_clk_freq / (1<<pwmscale);
  } else {
