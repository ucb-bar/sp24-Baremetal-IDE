...
float generate(Transformer *transformer, Tokenizer *tokenizer, Sampler *sampler, char *prompt, uint32_t steps) {
    char *empty_prompt = "";
    ...
    // Start Power Measurement Region of Interest (at iter 1)
    ...
    while (pos < steps) {
        ...
        // We want to skip this and instead just keep running the flow for benchmarking.
        // if (next == 1) { break; }
        ...
        if (start == 0) { start_roi(); start = READ_CSR("mcycle"); }
    }
    // printf("\r\n");

    // report achieved tok/s (pos-1 because the timer starts after first iteration)
    // Test concluded. Return tok/s and cycles
    unsigned long end = READ_CSR("mcycle");

    // End Power Measurement Region of Interest
    end_roi();
...
int main(int argc, char **argv) {
  ...
  uint32_t steps = 128;            // number of steps to run for (default 512)
  ...
  while (1) {
    // Host->Chip Payload Expectation:
    //   4-byte `steps` value
    
    // Host->Chip Communication
